...
/*
	...
	Reader는 문자기반의 입력스트림의 최상위클래스이다. 
	모든 문자기반 입력스트림 FileReader, BufferedReader, InputStreamReader는 
	모두 Reader클래스를 상속한다.
...
public class ReaderMain {
	...
	public static void main(String[] args) throws Exception {
		...
		while(true) {
			readCharNo = reader.read(c_arr); 
			if(readCharNo == -1) break;
			data += new String(c_arr, 0,  readCharNo);
		}
		reader.close();
		...
		reader = new FileReader("src/com/lec/ex01_input/news.txt");
		int readNo;
		char[] c_arr1 = new char[8];
		readNo = reader.read(c_arr1, 1, 4);
		for(int i=0;i<c_arr1.length;i++) {
			System.out.println(c_arr1[i]);
		}
		reader.close();
	}
}